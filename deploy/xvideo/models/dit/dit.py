...
_FP8_IMG_ENABLED = _env_on("STREAMEDIT_FP8_IMG")
_FP8_TXT_ENABLED = _env_on("STREAMEDIT_FP8_TXT")
...
from xvideo.models.dit import fused_ops as _fused
...
_SAGE_ATTN_ENABLED = _env_on("STREAMEDIT_SAGE_ATTN")
...
if _SAGE_ATTN_ENABLED:
    try:
        ...
        _sage_attn_func = None
        print(f"[dit] STREAMEDIT_SAGE_ATTN=1 but sageattention unavailable: {_sage_exc!r}")
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        img_modulated = _fused.fused_layernorm_modulate(
        ...
        if kv_cache_pre_rope:
            img_k_for_cache = _fused.rmsnorm_qk_bf16(
                img_k, self.img_attn_k_norm.weight, eps=self.img_attn_k_norm.eps
            )
        img_q, img_k = _fused.fused_qk_norm_rope_3d(
            img_q, img_k,
        ...
        if not skip_text_stream:
            txt_modulated = _fused.fused_layernorm_modulate(
        ...
        def _txt_tail(txt_in):
            t = _fused.fused_add_gate(
                txt_in, _txt_proj_call(txt_attn),
                txt_mod1_gate,
            )
            t_mod2 = _fused.fused_layernorm_modulate(
            ...
            return _fused.fused_add_gate(
                t, _txt_mlp_call(t_mod2),
                txt_mod2_gate,
            )

        img = _fused.fused_add_gate(
            img, _img_proj_call(img_attn),
            img_mod1_gate,
        )
        img_mod2_modulated = _fused.fused_layernorm_modulate(
