...
class StreamEditRuntime:
    ...
    def load(
        ...
        *,
        ...
        try:
            if os.environ.get("STREAMEDIT_SKIP_LOAD_WARMUP", "0").lower() in {"1", "true", "yes", "on"}:
                print("#####[STREAM] load-time warmup skipped (STREAMEDIT_SKIP_LOAD_WARMUP); "
    ...
    ) -> "StreamEditV2VStreamingSession":
        return StreamEditV2VStreamingSession(
            runtime=self,
            prompt=prompt,
...
class StreamEditV2VStreamingSession:
    _session_serial_counter = 0
    def __init__(
        self,
        *,
        runtime: StreamEditRuntime,
        prompt: str,
        settings: StreamingSettings,
        ref_image: Image.Image | None = None,
    ):
        self.runtime = runtime
        StreamEditV2VStreamingSession._session_serial_counter += 1
        self._session_serial = StreamEditV2VStreamingSession._session_serial_counter
    ...
    def _start_async_workers(self) -> None:
        ...
        self._workers = [
            threading.Thread(target=self._encode_worker, name="streamedit-vae-encode", daemon=True),
            threading.Thread(target=self._dit_worker, name="streamedit-dit-denoise", daemon=True),
            threading.Thread(target=self._decode_worker, name="streamedit-vae-decode", daemon=True),
            threading.Thread(target=self._pseudo_worker, name="streamedit-pseudo-encode", daemon=True),
            threading.Thread(target=self._postprocess_worker, name="streamedit-postprocess", daemon=True),
        ]
        ...
        return {
            ...
            "workers": {
                worker.name: {
                    ...
                    "state": worker_states.get(worker.name.replace("streamedit-", ""), {}),
                }
                for worker in self._workers
            },
